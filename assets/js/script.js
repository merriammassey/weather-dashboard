...
var displayData = function (data, location) {
  ...
  document.querySelector("#city-date").innerHTML =
    location +
    " " +
    luxon.DateTime.now().toLocaleString(luxon.DateTime.DATE_SHORT); //set image source to ("http://openweathermap.org/img/wn/10d@2x.png") //data.current.weather[0].icon;
  ...
  document.querySelector("#current-uv").innerHTML =
    "<p id='current-text'> UV Index: " + data.current.uvi + "</p>";
  ...
  if (data.current.uvi <= 5) {
    ...
    document
      .querySelector("#current-text")
      .setAttribute("style", "background-color: lavender;");
  }
  /*
    for (var i=0; i<5; i++) {
  ...
  document.querySelector("#date1").innerHTML = luxon.DateTime.now()
    .plus({ days: 1 })
  ...
  document.querySelector("#date2").innerHTML = luxon.DateTime.now()
    .plus({ days: 2 })
    .toLocaleString(luxon.DateTime.DATE_SHORT);
  ...
  document.querySelector("#date3").innerHTML = luxon.DateTime.now()
    .plus({ days: 3 })
    .toLocaleString(luxon.DateTime.DATE_SHORT);
  ...
  document.querySelector("#date4").innerHTML = luxon.DateTime.now()
    .plus({ days: 4 })
    .toLocaleString(luxon.DateTime.DATE_SHORT);
  ...
  document.querySelector("#date5").innerHTML = luxon.DateTime.now()
    .plus({ days: 5 })
    .toLocaleString(luxon.DateTime.DATE_SHORT);
...
var getWeatherData = function (location, latlng) {
  ...
  fetch(
    "https://api.openweathermap.org/data/2.5/onecall?" +
      latlng +
      "&appid=3facb0dc69ce03b89e13bc8c8ec66d00&units=imperial"
...
var getLocation = function (location) {
  ...
  fetch(
    "https://maps.googleapis.com/maps/api/geocode/json?address=" +
      location +
      "&key=AIzaSyBpq8U7q9Hz_ARre9lhkSRKKISOfPwlLZc"
  ).then(function (response) {
